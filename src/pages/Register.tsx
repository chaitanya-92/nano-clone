...
export default function Register() {
  ...
  return (
    <Formik
      ...
      initialValues={{
        ...initialValues,
        role,
        name: googleOnboarding
          ? user?.name ?? ""
          : initialValues.name,
        email: googleOnboarding
          ? user?.email ?? ""
          : initialValues.email,
      }}
      onSubmit={() =>
 ...
 const next=async()=>{if(!(await validateStep()))return;setSaving(true);try{if (
      step === 0 &&
      !googleOnboarding
    ) {
