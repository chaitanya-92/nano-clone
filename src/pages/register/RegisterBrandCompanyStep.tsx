...
export function RegisterBrandCompanyStep() {
  const { formik, analysis, setAnalysis, saving, setError } =
    useRegistrationForm();
    ...
    <div className="space-y-5">
      ...
      <button
        ...
        onClick={async () => {
          setError("");
          try {
            const result = await analyzeCompanyWebsite(formik.values.website);
            setAnalysis(result.data);
            if (result.data?.company_name)
              formik.setFieldValue("companyName", result.data.company_name);
            if (result.data?.description)
              formik.setFieldValue("description", result.data.description);
          } catch (e) {
            setError(
              e instanceof Error ? e.message : "Unable to analyze the website.",
            );
          }
        }}
