...
export function RegisterBrandIcpStep() {
  ...
  return (
    <div className="space-y-5">
      ...
      />
      <div>
        <p className="mb-2 text-xs font-semibold text-[#626a78]">Industries</p>
        <div className="flex flex-wrap gap-2">
          {industries.map((item) => {
            const selected = formik.values.industries.includes(item);
            return (
              <button
                key={item}
                type="button"
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {formik.values.icps.map(
          (item: RegistrationValues["icps"][number], index: number) => (
            <div
              key={index}
