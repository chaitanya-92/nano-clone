...
export default function Settings() {
  ...
  const [linkedin, setLinkedin] = useState<SocialField>({
    url: "",
    status: "idle",
    message: "",
  });
  ...
  const [xProfile, setXProfile] = useState<SocialField>({
    url: "",
    status: "idle",
    message: "",
  });
  ...
  useEffect(() => {
    ...
    void getCreatorProfile()
      .then(({ data }) => {
        ...
        const xUrl = data.x_profile_url ?? "";

        setLinkedin({
          url: linkedinUrl,
          status: linkedinUrl ? "valid" : "idle",
          message: linkedinUrl ? "Saved profile link." : "",
        });
        setInitialLinkedin(linkedinUrl);

        setXProfile({
          url: xUrl,
          status: xUrl ? "valid" : "idle",
          message: xUrl ? "Saved profile link." : "",
        });
        setInitialX(xUrl);
      })
      .catch((value) => {
  ...
  const hasChanges =
    ...
    linkedin.url.trim() !== initialLinkedin ||
    xProfile.url.trim() !== initialX;

  const saveChanges = async () => {
    const nextName = name.trim();
    const nextLinkedin = linkedin.url.trim();
    const nextX = xProfile.url.trim();
    ...
    const linkedinError = validateSocialUrl("linkedin", nextLinkedin);
    const xError = validateSocialUrl("x", nextX);
    ...
    try {
      if (nextLinkedin !== initialLinkedin) {
        setLinkedin((current) => ({
          ...current,
          status: "checking",
          message: "Checking public LinkedIn profile…",
        }));
        ...
        setLinkedin({
          ...
          status: "valid",
          message: result.data.fetchedProfile
      ...
      if (nextX !== initialX) {
        setXProfile((current) => ({
          ...current,
          status: "checking",
          message: "Checking public X profile…",
        }));
      ...
      setInitialName(data.name ?? nextName);

      const savedLinkedin = data.linkedin_url ?? nextLinkedin;
      const savedX = data.x_profile_url ?? nextX;

      setLinkedin({
        url: savedLinkedin,
        status: savedLinkedin ? "valid" : "idle",
        message: savedLinkedin ? "Saved profile link." : "",
      });
      setInitialLinkedin(savedLinkedin);

      setXProfile({
        url: savedX,
        status: savedX ? "valid" : "idle",
        message: savedX ? "Saved profile link." : "",
      });
      setInitialX(savedX);
      setSaved(true);
      ...
      setLinkedin((current) => ({
        ...current,
        status:
          current.url && current.message.includes("Checking")
            ? "invalid"
            : current.status,
      }));
  ...
  const addMethod = async () => {
    ...
    if (!value) {
      setError("Enter a payout method name.");
      return;
    }
    ...
    return (
      <div className="mx-auto w-full max-w-[1180px]">
        <div className="rounded-xl border border-[#f1c7c7] bg-[#fff7f7] px-4 py-3 text-sm text-[#9b3e3e]">
          {error || "Creator settings are unavailable."}
        </div>
      </div>
    );
  }
            ...
            <>
              <div className="border-b border-[#e8ecf2] pb-5">
                ...
                <p className="mt-1 text-sm text-[#7d899f]">
                  Keep your account identity and public social links in sync.
                </p>
              </div>
              ...
              <div className="space-y-8 pt-6">
                ...
                <div className="border-t border-[#e8ecf2] pt-7">
                  ...
                  <p className="mt-1 text-sm text-[#7d899f]">
                    Use public profile links. They are validated before the
                    changes are saved.
                  </p>
                    ...
                    <div className="flex items-center justify-between gap-4">
                      ...
                      {!methodLabel && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            setMethodLabel("Primary payout account")
                          }
                    ...
                    {methodLabel && (
                      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                        ...
                        <Button
                          ...
                          disabled={addingMethod || !methodLabel.trim()}
                          className="cursor-pointer bg-[#171d2b] hover:bg-[#111827]"
                        >
                          {addingMethod ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 max-w-[700px] text-sm leading-6 text-[#8d6262]">
                      Permanently delete your Naano account and associated
        ...
        <DialogContent className="!max-w-[520px] border-[#ecd1d1] p-6">
          <DialogHeader>
            ...
            <DialogDescription className="leading-6">
              This permanently deletes your account and associated workspace
          ...
          <div className="rounded-xl border border-[#f0d4d4] bg-[#fff8f8] px-4 py-3">
            <p className="text-xs leading-5 text-[#8a5d5d]">
              Type DELETE below to enable the confirmation button.
            </p>
          </div>
...
function SocialField({
  ...
  const localError = validateSocialUrl(provider, value.url);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3">
        <label htmlFor={id} className="text-xs font-semibold text-[#626a78]">
          {label}
        </label>

        <span className="text-[11px] font-semibold">
          {value.status === "valid" ? (
            <span className="text-[#188b56]">{statusLabel}</span>
          ) : value.status === "invalid" ? (
            <span className="text-[#c24d4d]">{statusLabel}</span>
          ) : (
            <span className="text-[#8b97aa]">{statusLabel}</span>
          )}
        </span>
      </div>
      ...
      <div className="relative">
        <input
          ...
          className={
            "auth-input pr-10 " +
            (localError || value.status === "invalid"
