...
function CreatorVisual({ visual }: { visual: CreatorVisualData }) {
  return (
    <div className="flex w-full items-center justify-center">
      <div className="flex items-start justify-center gap-2">
        {visual.creators.map((creator, index) => (
          <motion.div
            key={creator.name}
...
function BriefVisual({ visual }: { visual: BriefVisualData }) {
  return (
    <motion.div
...
function CollaborationVisual({ visual }: { visual: CollaborationVisualData }) {
  return (
    <motion.div
      ...
      viewport={{ once: true, amount: 0.45 }}
      transition={revealTransition}
      className="w-[166px] rounded-[16px] border border-[#e3edf3] bg-white px-4 py-3.5 shadow-[0_10px_30px_rgba(40,90,120,0.07)]"
    >
        ...
        {visual.creators.map((creator, index) => (
          ...
          >
            ...
            <motion.span
              ...
              className="rounded-md bg-[#e5f3fb] px-1.5 py-1 text-[6px] font-semibold leading-none text-[#4d7897]"
            >
              {creator.status}
            </motion.span>
          </motion.div>
...
function ResultsVisual({ visual }: { visual: ResultsVisualData }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10, scale: 0.97 }}
      whileInView={{ opacity: 1, y: 0, scale: 1 }}
    ...
    >
      ...
      <div className="mt-3 flex justify-between text-[6.5px] leading-none text-[#9ba2aa]">
        ...
        <span>{visual.leads}</span>
      </div>
    </motion.div>
  );
}

function PaymentVisual({ visual }: { visual: PaymentVisualData }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 10, scale: 0.97 }}
      whileInView={{ opacity: 1, y: 0, scale: 1 }}
    ...
    >
      ...
      <div className="mt-3 flex gap-1.5">
        {visual.actions.map((action, index) => (
          <motion.span
            key={action}
...
function WorkflowVisual({ step }: { step: WorkflowStep }) {
  switch (step.visual.type) {
    case "creators":
      return <CreatorVisual visual={step.visual} />;
    case "brief":
      return <BriefVisual visual={step.visual} />;
    case "collaboration":
      return <CollaborationVisual visual={step.visual} />;
    case "results":
      return <ResultsVisual visual={step.visual} />;
    case "payment":
      return <PaymentVisual visual={step.visual} />;
    default:
      return null;
  }
}
...
export function WorkflowCard({ step, index = 0 }: WorkflowCardProps) {
  return (
    <motion.article
    ...
    >
      <div className="absolute -top-[13px] left-[14px] z-30 grid h-7 w-7 place-items-center rounded-full border border-[#d9eaf3] bg-[#f7fcff] text-[8px] font-semibold leading-none text-[#5f8096] shadow-[0_2px_8px_rgba(55,100,125,0.04)]">
        {step.number}
      </div>

      <div className="flex min-h-[218px] flex-1 items-center justify-center">
        <WorkflowVisual step={step} />
      </div>

      <h3 className="mt-auto max-w-[205px] text-[15px] font-semibold leading-[1.18] tracking-[-0.025em] text-[#171a1e]">
        {step.title}
      </h3>
    </motion.article>
  );
