...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed right-4 top-4 z-[100] mx-0 w-[calc(100vw-2rem)] max-w-sm outline-none sm:right-4 sm:w-full",
        className,
      )}
      {...props}
    />
...
function Toast({ className, ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
      className={cn(
        "pointer-events-auto absolute right-0 top-0 z-[calc(1000-var(--toast-index))] w-full overflow-hidden rounded-2xl border bg-white text-[#171d2b] shadow-[0_18px_50px_rgba(23,29,43,.18)] outline-none",
