...
function allowed(user: any, row: any) {
  return user.id === row.creator_id || user.id === row.brand_id;
}
...
export function getCollaboration(
  ...
  const row = db
    .prepare(
      "SELECT c.*,ca.title AS campaign_title,u.name AS creator_name,bp.company_name AS brand_name FROM collaborations c JOIN campaigns ca ON ca.id=c.campaign_id JOIN users u ON u.id=c.creator_id JOIN brand_profiles bp ON bp.user_id=c.brand_id WHERE c.id=?",
    )
    .get(id) as any;
  if (!row)
    return error(
...
export async function updateCollaboration(
  ...
  const timestamp = now();
  db.prepare(
    "UPDATE collaborations SET status=?,brief=?,content_url=?,published_url=?,approved_at=CASE WHEN ?='content_approved' THEN ? ELSE approved_at END,completed_at=CASE WHEN ?='completed' THEN ? ELSE completed_at END,updated_at=? WHERE id=?",
  ).run(
    status,
    brief,
    contentUrl,
  ...
  if (status === "payment_released") {
    ...
    if (!exists) {
      ...
      db.prepare(
        "INSERT INTO earnings (id,user_id,collaboration_id,type,status,amount_cents,currency,description,available_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
      ).run(
        require("node:crypto").randomBytes(16).toString("base64url"),
        row.creator_id,
        id,
        "collaboration",
        "available",
        amount?.budget_cents ?? 0,
        "EUR",
